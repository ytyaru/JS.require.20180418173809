...
名前|en|表記|説明
----|--|----|----
押す|press|`[A]`|同一のキーを押下してから離すまでの工程
押下する|down|`[A]`|キーを押下する
離す|up|`[A]`|押下しているキーを離す
連押し||`[[A]*2]`,`[A(2)]`,`[A^2]`|同一キーを高速で2回押す `*`は順不同押下押しと被る
順押し||`[[A][B]]`,`[A,B]`|複数のキーを順にpressする
長押し||`[[A]1s]`,`[A(1s)]`,`[A 1s]`,`[A 0.2s],`[A .2s]`,`[A 200ms]`|一定時間downしたあとに離す
溜め押し||`[A 1.5..2s]`|一定時間downしたあとに離す
溜め押し||`[A 1.5+-.2s]`|1.3..1.7秒間
同時押し||`[[A]=[B]]`,`[A=B]`,`[A=B=C]`,`[=A,B,C]`|2つ以上のキーを同時に押す
順押下(押下押し)||`[[A]+[B]]`,`[A+B]`,`[A+B+C]`,`[+A,B,C]`|あるキーをdown中に別のキーを押下する
順不同押下押し|hold|`[[A]*[B]]`,`[A*B]`,`[A*B*C]`,`[*A,B,C]`|順不同の複数押下。
どれか||`[A|B]`|`|`で区切ったどれかの入力。
